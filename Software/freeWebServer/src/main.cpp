...
/**
 * @brief Flag for tuning PID constants.
 *
 */
...
{
    WebServerManager webServer(wifiConfig, WIFI_PASS_NR);

    if (webServer.begin())
    {
        webServer.setupWiFIRouter(wifiConfig);
    }
...
void pidTaskHandle(void *pvParameters)
{
    ...
    double insideTemp = static_cast<double>(currentTemperature.inside_temperature);
    double setpointTemp = static_cast<double>(currentTemperature.setpoint_temperature);

    PID_ATune aTune(&insideTemp, &setpointTemp);
    ...
    for (;;)
    {
        ...
        if (LOG_MESSAGE && xTaskGetTickCount() - lastTime > LOGS_OFFSET)
        {
            ...
            Serial.println("PID stack high water mark: " + String(uxTaskGetStackHighWaterMark(NULL)));
            lastTime = xTaskGetTickCount();
        }
